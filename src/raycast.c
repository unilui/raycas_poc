...
/*   Updated: 2023/03/18 21:52:28 by lufelip2         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static t_hit	raycast(t_cub *rcast, double ray_angle)
{
	double	dx;
	double	dy;
	t_ray	ray;
	...
	hit = (t_hit){-1, -1};
	wall = '0';
	ray = (t_ray){(rcast->player.x), (rcast->player.y)};
	...
	while (wall != '1')
	{
		ray.x += dx;
		ray.y += dy;
		wall = rcast->map.rows[(int)floor(ray.y)][(int)floor(ray.x)];
	}
	hit.x = ray.x;
	hit.y = ray.y;
	hit.side = get_hit_side(&rcast->player, &hit, ray_angle);
	hit.distance = sqrt(pow(rcast->player.x - ray.x, 2)
			+ pow(rcast->player.y - ray.y, 2));
